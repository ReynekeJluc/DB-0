...
class BrandController {
	...
	async manyFind(req, res) {
		try {
			...
			const limitBrands = !isNaN(parseInt(filters.limit)) ? parseInt(filters.limit) : defaultLimit; // Количество результатов (по умолчанию 5)
			const offsetBrands = !isNaN(parseInt(filters.offset)) ? parseInt(filters.offset) : defaultOffset; // Смещение (по умолчанию 0)
